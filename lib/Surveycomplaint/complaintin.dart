...
class Complaintcheck extends StatefulWidget {
  // final data;
  // Complaintcheck(this.data);

  @override
  _ComplaintcheckState createState() => _ComplaintcheckState();
}

// ignore: camel_case_types
class _ComplaintcheckState extends State<Complaintcheck> {
  var locationMessage = "";
  var lat = " ";
  var lng = " ";
  Position? posisition;
  ...
  void initState() {
    ...
    _getCurrentLocation();
    choiceImage();
    // getPref(_currentAddress);
  }

  getPref(_currentAddress) async {
    WidgetsFlutterBinding.ensureInitialized();
    SharedPreferences preferences = await SharedPreferences.getInstance();
    setState(() {
      var stts = preferences.getString("location");
    });
    await preferences.setString('location', _currentAddress);
  }
  ...
  Future upload(File imageFile) async {
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.pushReplacement(
          context, new MaterialPageRoute(builder: (context) => surveydone()));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          ...
          toolbarHeight: 80,
          title: Text(
            "Check In Complaint Survey Attendances",
            style: TextStyle(
